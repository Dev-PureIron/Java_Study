...
//   대댓글 삭제
   public void deleteReply(Long replyId) {
	   ...
	   try {
		...
		System.out.println("delete (Long replyId) SQL문 오류");
		e.printStackTrace();
	} catch (Exception e) {
        e.printStackTrace();
     } finally {
...
//   댓글 삭제
   public void deleteReReply(Long replygroupId) {
		   String query = "DELETE FROM TBL_REPLY"
		   		+ "WHERE REPLY_GROUP = ? ";
		   ...
		   try {
			preparedStatement = connection.prepareStatement(query);
			preparedStatement.setLong(1, replygroupId);
			preparedStatement.executeUpdate();
...
//   댓글 수정
   public void update(ReplyVO replyVO) {
	   ...
	   try {
		...
		System.out.println("update (ReplyVO replyVO SQL문 오류");
		e.printStackTrace();
	} catch (Exception e) {
        e.printStackTrace();
     } finally {
